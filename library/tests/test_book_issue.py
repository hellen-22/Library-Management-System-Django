...
class TestLendBookView(TestCase):
    def setUp(self):
        ...
        self.data = {"book": self.book.pk, "member": self.member.pk, "return_date": "2024-12-12", "fine": 0.00, "payment_method": "cash"}
...
class TestLendIndividualMemberView(TestCase):
    ...
    def test_login_required(self):
        ...
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('issue-member-book', kwargs={'pk': self.member.pk})}")
    ...
    def test_invalid_data_does_not_lend_book(self):
        self.client.force_login(self.user)
        self.client.post(reverse("issue-member-book", kwargs={"pk": self.member.pk}), {"book": "", "return_date": "", "fine": ""})
...
class TestUpdateLentBookView(TestCase):
    ...
    def test_login_required(self):
        ...
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('edit-borrowed-book', kwargs={'pk': self.borrowed_book.pk})}")
    ...
    def test_invalid_data_does_not_update_borrowed_book(self):
        self.client.force_login(self.user)
        self.client.post(reverse("edit-borrowed-book", kwargs={"pk": self.borrowed_book.pk}), {"return_date": "", "fine": ""})
...
class TestDeleteLentBookView(TestCase):
    ...
    def test_login_required(self):
        ...
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('delete-borrowed-book', kwargs={'pk': self.borrowed_book.pk})}")
        self.assertEqual(BorrowedBook.objects.count(), 1)
