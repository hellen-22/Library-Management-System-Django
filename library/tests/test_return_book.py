...
class TestReturnBookView(TestCase):
    ...
    def test_login_required(self):
        ...
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('return-book', kwargs={'pk': self.borrowed_book2.pk})}")
...
class TestReturnBookOnFineView(TestCase):
    ...
    def test_login_required(self):
        ...
        self.assertEqual(self.borrowed_book.returned, False)
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('return-book-fine', kwargs={'pk': self.borrowed_book.pk})}")
